...
import { LogOut, RefreshCw, AlertCircle, Clock, CheckCircle2, XCircle, ChevronDown, ChevronUp } from "lucide-react";
...
export function AdminDashboard() {
  ...
  const newCount = requests.filter((r) => !r.viewed_at).length;
    ...
    <div className="min-h-screen bg-[#F7F7F7]">
      ...
      <header className="bg-white border-b border-[#D8D8D8] sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-5 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={fetchRequests} className="p-2 text-[#5E6168] hover:text-[#050505] transition-colors" aria-label="Refresh">
              <RefreshCw size={16} />
            </button>
      ...
      <div className="max-w-6xl mx-auto px-5 py-6">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <div className="flex gap-1 bg-white border border-[#D8D8D8] rounded-xl p-1">
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={() => { onToggle(); handleMarkViewed(); }}
        role="button"
        tabIndex={0}
        onKeyDown={(e) => { if (e.key === "Enter") { onToggle(); handleMarkViewed(); } }}
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            <span className={`text-[10px] font-bold uppercase tracking-wider rounded-full px-2 py-0.5 ${priorityColor}`}>
              {req.priority}
            </span>
            <span className="text-xs font-mono text-[#9A9DA5]">{req.request_number}</span>
          </div>
          <p className="font-bold text-[#050505] text-sm leading-snug truncate">{req.item_request}</p>
          <p className="text-xs text-[#5E6168] mt-0.5">
            {req.full_name}{req.company_name ? ` · ${req.company_name}` : ""} · {req.budget} · {req.delivery_location}
          </p>
        </div>
      ...
      {expanded && (
        <div className="border-t border-[#D8D8D8] px-4 py-5 flex flex-col gap-5">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            ...
            <Detail label="Budget" value={req.budget + (req.budget_numeric ? ` (~$${req.budget_numeric.toLocaleString()})` : "")} />
            <Detail label="Deadline" value={req.deadline_is_flexible ? "Flexible" : req.deadline ?? "—"} />
            <Detail label="Location" value={req.delivery_location} />
            <Detail label="Score" value={req.ai_score != null ? `${req.ai_score}/10 (AI)` : req.fallback_score != null ? `${req.fallback_score}/10 (fallback)` : "—"} />
            <Detail label="Source" value={req.source ?? "—"} />
            <Detail label="Created" value={new Date(req.created_at).toLocaleString()} />
          </div>

          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#9A9DA5] mb-1">Request</p>
            <p className="text-sm text-[#050505] leading-relaxed bg-[#F7F7F7] rounded-xl p-3">{req.item_request}</p>
          </div>

          {req.additional_details && (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#9A9DA5] mb-1">Additional Details</p>
              <p className="text-sm text-[#5E6168] leading-relaxed">{req.additional_details}</p>
            </div>
          )}
          ...
          {(req.ai_summary || req.fallback_score != null) && (
            <div className="bg-blue-50 rounded-xl p-3">
              ...
              {req.ai_summary && <p className="text-sm text-[#050505] leading-relaxed mb-2">{req.ai_summary}</p>}
              {req.ai_recommendation && (
                <p className="text-xs text-[#5E6168]">Recommendation: <strong>{req.ai_recommendation}</strong></p>
              )}
              ...
              {req.ai_recommended_questions && req.ai_recommended_questions.length > 0 && (
                <div className="mt-2">
                  <p className="text-xs font-bold text-[#5E6168] mb-1">Suggested questions:</p>
                  <ul className="text-xs text-[#5E6168] list-disc pl-4 space-y-0.5">
                    {req.ai_recommended_questions.map((q) => <li key={q}>{q}</li>)}
                  </ul>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex flex-col gap-1">
              ...
              <select
                value={req.status}
                onChange={(e) => onUpdate({ status: e.target.value as RequestStatus })}
                className="border border-[#D8D8D8] rounded-lg px-3 py-2 text-sm text-[#050505] bg-white focus:outline-none focus:border-[#FFC400]"
              >
                {(["new","reviewing","needs_information","potential_match","accepted","not_a_fit","closed"] as RequestStatus[]).map((s) => (
                  <option key={s} value={s}>{s.replace(/_/g, " ")}</option>
                ))}
              </select>
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold uppercase tracking-widest text-[#9A9DA5]">Priority</label>
              <select
                value={req.priority}
                onChange={(e) => onUpdate({ priority: e.target.value as RequestPriority })}
                className="border border-[#D8D8D8] rounded-lg px-3 py-2 text-sm text-[#050505] bg-white focus:outline-none focus:border-[#FFC400]"
              >
...
function StatusBadge({ status }: { status: RequestStatus }) {
  const styles: Record<RequestStatus, { color: string; icon: React.ReactNode }> = {
    new: { color: "bg-[#FFC400] text-[#050505]", icon: <Clock size={10} /> },
    reviewing: { color: "bg-blue-100 text-blue-700", icon: <RefreshCw size={10} /> },
    needs_information: { color: "bg-orange-100 text-orange-700", icon: <AlertCircle size={10} /> },
    potential_match: { color: "bg-purple-100 text-purple-700", icon: <CheckCircle2 size={10} /> },
    accepted: { color: "bg-green-100 text-green-700", icon: <CheckCircle2 size={10} /> },
    not_a_fit: { color: "bg-gray-100 text-gray-500", icon: <XCircle size={10} /> },
    closed: { color: "bg-gray-100 text-gray-400", icon: <XCircle size={10} /> },
  };
  const s = styles[status];
  return (
    <span className={`flex items-center gap-1 text-[10px] font-bold rounded-full px-2 py-0.5 ${s.color}`}>
      {s.icon}
      {status.replace(/_/g, " ")}
    </span>
  );
